...
# corpus directory and base url for downloads
data=/work/jtao/corpora/speech
data_url=www.openslr.org/resources/17
...
if [ $stage -le 0 ]; then
  ...
  if [ ! -f $data/rirs_noises.zip ]; then
    # Download the package that includes the real RIRs, simulated RIRs, isotropic noises and point-source noises
    wget --no-check-certificate -P $data http://www.openslr.org/resources/28/rirs_noises.zip
  elif [ ! -d $data/RIRS_NOISES ]; then
    unzip -d $data $data/rirs_noises.zip
  fi

  if [ ! -h RIRS_NOISES ]; then
    ln -s $data/RIRS_NOISES RIRS_NOISES
  fi
  ...
  rvb_opts=()
  rvb_opts+=(--rir-set-parameters "0.5, $data/RIRS_NOISES/simulated_rirs/smallroom/rir_list")
  rvb_opts+=(--rir-set-parameters "0.5, $data/RIRS_NOISES/simulated_rirs/mediumroom/rir_list")
...
if [ $stage -le 1 ]; then
  # Download the MUSAN corpus
  local/download_and_untar.sh $data $data_url "musan"

  # Prepare the MUSAN corpus, which consists of music, speech, and noise
  # We will use them as additive noises for data augmentation.
  steps/data/make_musan.sh $data/musan data
...
if [ $stage -le 3 ] && $generate_alignments; then
  ...
  steps/copy_ali_dir.sh --nj 50 --cmd "$train_cmd" \
  ...
  if [ $stage -le 10 ]; then
    ...
    for dataset in $test_sets; do
      steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj 10 \
        data/${dataset}_hires exp/nnet3/extractor exp/nnet3/ivectors_${dataset}_aug || exit 1;
    done
  fi
fi
